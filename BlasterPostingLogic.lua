...
function AL:HandlePostFailure(reason)
    ...
    -- [[ DIRECTIVE: Reduce post delay for faster blasting ]]
    C_Timer.After(0.5, function() AL:BlastNextItem() end)
end
...
function AL:AddScannedItemToQueue(itemID, postPrice, skipped, skipReason, undercutInfo)
    ...
    if link then
        -- [[ DIRECTIVE: Add readyForRescan flag ]]
...
function AL:BlastNextItem()
    ...
    if not AuctionHouseFrame or not AuctionHouseFrame:IsShown() then
        ...
        C_Timer.After(2.0, function() if not AL.isPosting and AL.blasterQueue and #AL.blasterQueue > 0 then AL:BlastNextItem() end end)
        return
    end
