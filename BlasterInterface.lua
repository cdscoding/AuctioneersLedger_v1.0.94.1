...
function AL:CreateUnderminePopup()
    ...
    p:SetScript("OnDragStart", function(self) self:StartMoving() end)
    p:SetScript("OnDragStop", function(self) self:StopMovingOrSizing() end)
    p.TitleText:SetText("Undermine Exchange Info")
    p.CloseButton:SetScript("OnClick", function() p:Hide() end)

    local disclaimerText = p:CreateFontString(nil, "ARTWORK", "GameFontNormalSmall")
    disclaimerText:SetPoint("TOP", p, "TOP", 0, -35)
    disclaimerText:SetWidth(p:GetWidth() - 40)
    disclaimerText:SetJustifyH("CENTER")
    disclaimerText:SetTextColor(0.8, 0.8, 0.8, 1)
    disclaimerText:SetText("CS&A Software is unaffiliated with Undermine Exchange but recommends it for finding prices on unlisted items.")
    p.DisclaimerText = disclaimerText

    local urlBox = CreateFrame("EditBox", nil, p, "InputBoxTemplate")
    urlBox:SetPoint("TOP", disclaimerText, "BOTTOM", 0, -10)
    urlBox:SetSize(280, 20)
    urlBox:SetText("https://undermine.exchange/")
    urlBox:SetAutoFocus(false)
    p.UrlBox = urlBox
    
    local instruction = p:CreateFontString(nil, "ARTWORK", "GameFontNormalSmall")
    instruction:SetPoint("TOP", urlBox, "BOTTOM", 0, -5)
    instruction:SetTextColor(1, 1, 0, 1)
    instruction:SetText("Press Ctrl+C to copy the URL.")
