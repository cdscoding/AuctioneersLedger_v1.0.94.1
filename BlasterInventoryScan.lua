...
function AL:LoadItemsToScan()
    ...
    if C_Container and type(C_Container.GetBagIDs) == "function" then
        ...
        if reagentBagId then
            ...
            for _, existingId in ipairs(bagIDs) do
                if existingId == reagentBagId then alreadyExists = true; break; end
            end
...
function AL:ProcessScanResult(scannedItem, itemKey, eventName)
    ...
    if not lowestCompetitorPrice then
        if normalBuyoutPrice > 0 then
            ...
            local skipReason = "No pricing data found. Right-click to set a price, or use the Undermine Exchange button to research its value."
